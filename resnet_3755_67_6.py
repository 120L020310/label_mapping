...
utils6.set_logger('train.log')

# Create the input data pipeline
parser = argparse.ArgumentParser()

parser.add_argument("--total_epoch", default=30, type=int, help="Total number of training epochs to perform.")
...
parser.add_argument('--restore_file', default="best_67_6", help="Optional, name of the file in --model_dir containing weights to reload before \
...
def train(args, model,train_loader,test_loader,optimizer,criterion,scheduler,restore_file=None):
    ...
    for i in range(args.total_epoch):
        ...
        # 每个epcoh保存一次模型参数
        # Save weights
        utils6.save_checkpoint({'epoch': i + 1,
                               'state_dict': model.state_dict(),
                               'optim_dict': optimizer.state_dict()},
                              is_best=is_best,
                              checkpoint=args.model_dir)

wandb.init(project='resnet_67_6', name=time.strftime('%m%d%H%M%S'))
...
train(args,net,train_loader,test_loader,optimizer,criterion,scheduler)