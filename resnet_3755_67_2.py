...
parser.add_argument('--restore_file', default="best_67_2", help="Optional, name of the file in --model_dir containing weights to reload before \
...
def train(args, model,train_loader,test_loader,optimizer,criterion,scheduler,restore_file=None):
    ...
    if restore_file is not None:
        ...
        utils2.load_checkpoint(restore_path, model, optimizer)
    ...
    for i in range(args.total_epoch):
        ...
        # Save weights
        utils2.save_checkpoint({'epoch': i + 1,
...
wandb.init(project='resnet_67_2', name=time.strftime('%m%d%H%M%S'))
