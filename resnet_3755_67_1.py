...
parser.add_argument('--restore_file', default="best_67_1", help="Optional, name of the file in --model_dir containing weights to reload before \
...
def train(args, model,train_loader,test_loader,optimizer,criterion,scheduler,restore_file=None):
    ...
    for i in range(args.total_epoch):
        ...
        # Save weights
        utils1.save_checkpoint({'epoch': i + 1,
...
wandb.init(project='resnet_67_1', name=time.strftime('%m%d%H%M%S'))
